...
export async function googleSearch(
  ...
  // Alternative methodology (recommended for Docker): use SearXNG JSON API
  // This avoids Playwright/Chromium automation and therefore avoids Google CAPTCHA.
  ...
  if (searxngBaseUrl) {
    ...
    try {
      ...
      logger.info(`[GoogleSearch] Using SearXNG backend: ${base}`);
      logger.info(`[GoogleSearch] SearXNG request: ${url}`);
      ...
      if (!res.ok) {
        ...
        throw new Error(
          `SearXNG search failed: HTTP ${res.status} ${res.statusText}${body ? ` - ${body}` : ""}`
        );
      }
      ...
      // If SearXNG is configured but unavailable, fall back to Playwright to preserve functionality.
      const msg = e?.name === "AbortError" ? "SearXNG request timed out" : String(e?.message || e);
      logger.warn(`[GoogleSearch] SearXNG backend failed, falling back to Playwright: ${msg}`);
    } finally {
      clearTimeout(timer);
    }
  }
  ...
  // NOTE: We force the use of the primary .com domain to avoid DNS issues
  // in restricted environments (some country-specific TLDs may not resolve).
  const PRIMARY_GOOGLE_DOMAIN = "https://www.google.com";
  ...
  async function performSearch(headless: boolean): Promise<SearchResponse> {
    // Check if we're in a Docker/headless environment (no X server available)
    const isDockerEnvironment = !process.env.DISPLAY || process.env.DISPLAY === '';
    ...
    try {
      // Always prefer the primary .com domain to avoid DNS issues with
      // country-specific TLDs (e.g., .com.au) in some environments.
      let selectedDomain: string = PRIMARY_GOOGLE_DOMAIN;

      // If we have a previously saved domain and it is still the primary .com,
      // we can reuse it; otherwise, override it with the primary domain.
      if (
        savedState.googleDomain &&
        savedState.googleDomain.includes("https://www.google.com")
      ) {
        selectedDomain = savedState.googleDomain;
        logger.info(
          `[GoogleSearch] Using saved Google domain (validated as .com): ${selectedDomain}`
        );
      } else {
        savedState.googleDomain = PRIMARY_GOOGLE_DOMAIN;
        logger.info(
          `[GoogleSearch] Using primary Google domain: ${PRIMARY_GOOGLE_DOMAIN}`
        );
      }

      logger.info(`[GoogleSearch] Preparing to search for: ${query}`);

      // PRIMARY APPROACH: Navigate directly to search URL (avoids cookie consent banner and is less detectable)
      // URL encode the query
      const encodedQuery = encodeURIComponent(query);
      const searchUrl = `${selectedDomain}/search?q=${encodedQuery}`;
      
      logger.info(`[GoogleSearch] Navigating directly to search URL: ${searchUrl}`);
      
      // Navigate directly to search results page (bypasses cookie consent banner)
      const response = await page.goto(searchUrl, {
        timeout,
        waitUntil: "domcontentloaded", // Use domcontentloaded instead of networkidle for faster loading
      });
      
      logger.info("[GoogleSearch] Direct navigation successful, waiting for results to load...");
      ...
      if (isBlockedPage) {
        // Check if we're in a Docker/headless environment (no X server)
        const isDockerEnvironment = !process.env.DISPLAY || process.env.DISPLAY === '';
        
        if (headless && !isDockerEnvironment) {
          ...
          // In Docker, we can't use non-headless mode, so throw an error
          logger.error("[GoogleSearch] CAPTCHA detected in Docker environment (no X server). Cannot switch to non-headless mode.");
          throw new Error("Google CAPTCHA detected. The search cannot be completed automatically in this environment. Please try again later or use a different search method.");
        } else {
      ...
      // Check if search URL redirected to CAPTCHA page after navigation
      ...
      if (isBlockedAfterSearch) {
        ...
        if (headless && !isDockerEnvironment) {
          ...
          // In Docker, we can't use non-headless mode, so throw an error
          logger.error("[GoogleSearch] CAPTCHA detected after search in Docker environment (no X server). Cannot switch to non-headless mode.");
          throw new Error("Google CAPTCHA detected after search. The search cannot be completed automatically in this environment. Please try again later or use a different search method.");
        } else {
      ...
      if (!resultsFound) {
        ...
        if (isBlockedDuringResults) {
          ...
          if (headless && !isDockerEnvironment) {
            ...
            // In Docker, we can't use non-headless mode, so throw an error
            logger.error("[GoogleSearch] CAPTCHA detected while waiting for results in Docker environment (no X server). Cannot switch to non-headless mode.");
            throw new Error("Google CAPTCHA detected while waiting for results. The search cannot be completed automatically in this environment. Please try again later or use a different search method.");
          } else {
            logger.warn(
